...
Figure 25.7.2: Shopping spree in which a user can fit 3 items in a shopping bag.
#include <iostream>
#include <string>
#include <vector>
using namespace std;
...
void ReorderNums(vector<int> remainNums, vector<int> reorderNums) {
   vector<int> tmpRemainNums;
   int tmpRemovedNum;
   int i;

   if (remainNums.size() == 0) {
      cout << reorderNums.at(0);
      cout << reorderNums.at(1);
      cout << reorderNums.at(2) << endl;
   }
   else {
      for (i = 0; i < remainNums.size(); ++i) {
         ...
         reorderNums.push_back(tmpRemovedNum);
         ReorderNums(tmpRemainNums, reorderNums);
         reorderNums.erase(reorderNums.end() - 1); // Remove last element
      }
   }
}

int main() {
   vector<int> numsToReorder;
   vector<int> resultNums;

   numsToReorder.push_back(5);
   numsToReorder.push_back(6);
   numsToReorder.push_back(8);
